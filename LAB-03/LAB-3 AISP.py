...
plt.subplot(2, 1, 1)
plt.plot(np.arange(len(y)) / sr, y, color='red')
plt.title('Original Audio')
plt.xlabel('Time (s)')
plt.ylabel('Amplitude')
 
plt.subplot(2, 1, 2)
plt.plot(np.arange(len(trimmed_y)) / sr, trimmed_y, color='yellow')
...
top_db_values = [10, 20, 30, 5]  # different top_db values
...
for i, top_db in enumerate(top_db_values):
    ...
    plt.plot(np.arange(len(y)) / sr, y, color='yellow')
    plt.title(f'Original Audio with top_db={top_db}')
    plt.xlabel('Time (s)')
    plt.ylabel('Amplitude')
 
    for interval in intervals:
        plt.axvline(interval[0] / sr, color='blue', linestyle='--')
        plt.axvline(interval[1] / sr, color='blue', linestyle='--')
...
for i, top_db in enumerate(top_db_values):
    ...
    for interval in split_audio_intervals:
        plt.axvline(interval[0], color='yellow', linestyle='-', alpha=0.7)
        plt.axvline(interval[1], color='yellow', linestyle='-', alpha=0.7)
